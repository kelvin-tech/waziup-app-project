...
const create = (baseURL = AppConfig.apiUrl) => {
  ...
  const apiInstance = new WaziupApi.ProjectsApi();
  ...
  //await getAuthToken, getProjectPermisssions, getGatewayPermissions and getDevicePermissions
  const getAuthToken = (credentials) => apiInstance.getAuthToken(credentials)
  const getProjectPermissions = () => apiInstance.getProjectPermissions()
  const getDevicePermissions = () => apiInstance.getAuthToken()
  const getGatewayPermissions = () => apiInstance.getAuthToken()
  ...
  const getUser = (userId) => apiInstance.get('api/users/' + userId)
  const getUsers = (options) => apiInstance.get('api/users', options)
